...
then
	...
	then
		...
		make -C buildroot O=${OUTPUT_DIR} defconfig BR2_EXTERNAL=${EXTERNAL_REL_BUILDROOT} BR2_DEFCONFIG=${AESD_MODIFIED_DEFCONFIG_REL_BUILDROOT} BR2_DL_DIR=${DL_DIR}
	else
	...
	make -C buildroot O=${OUTPUT_DIR} BR2_EXTERNAL=${EXTERNAL_REL_BUILDROOT}
